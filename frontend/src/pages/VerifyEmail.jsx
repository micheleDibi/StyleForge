...
const VerifyEmail = () => {
  ...
  const [message, setMessage] = useState(token ? '' : 'Token mancante nel link.');
  ...
  useEffect(() => {
    ...
    verifyEmail(token)
      .then((d) => {
        if (!cancelled) { setState('success'); setMessage(d.message || 'Email verificata con successo.'); }
      })
      .catch((err) => {
        if (!cancelled) { setState('error'); setMessage(err.response?.data?.detail || 'Link di verifica non valido o scaduto.'); }
      });
      ...
      <div className="w-full max-w-md glass rounded-3xl p-8 shadow-2xl text-center">
        ...
        {state === 'success' && (
          <>
            ...
            <h1 className="text-xl font-bold text-slate-900 mb-2">Email verificata</h1>
            <p className="text-slate-600 mb-6">{message}</p>
            <Link to="/login" className="btn btn-primary btn-lg w-full gap-2">
              <LogIn className="w-4 h-4" /> Vai al login
            </Link>
          </>
        )}
        {state === 'error' && (
          <>
            ...
            <h1 className="text-xl font-bold text-slate-900 mb-2">Verifica non riuscita</h1>
            <p className="text-slate-600 mb-6">{message}</p>
