...
const PaymentReturn = () => {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (cancelled) return;
        setError(err?.response?.data?.detail || 'Errore nel recupero dello stato pagamento.');
        setPolling(false);
      }
    };
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-6">
      <h1 className="text-2xl font-bold text-slate-900">Esito pagamento</h1>

      {error && (
        <div className="glass rounded-2xl p-6 flex items-start gap-3">
          <AlertTriangle className="w-6 h-6 text-red-500 flex-shrink-0" />
          <div>
            <h2 className="font-semibold text-slate-900">Errore</h2>
      ...
      {!error && polling && (
        <div className="glass rounded-2xl p-8 flex flex-col items-center text-center space-y-3">
          ...
          <h2 className="font-semibold text-slate-900">Conferma pagamento in corso…</h2>
          <p className="text-sm text-slate-500">
            Stiamo aspettando la notifica di esito da PagoPA. Di solito sono pochi secondi.
          </p>
          {elapsed > 0 && (
            <p className="text-xs text-slate-400">{Math.round(elapsed / 1000)}s</p>
          )}
        </div>
      )}
      ...
      {!error && !polling && order?.status === 'PAID' && (
        <div className="glass rounded-2xl p-8 flex flex-col items-center text-center space-y-4 border-2 border-emerald-200 bg-emerald-50/50">
          ...
          <div>
            <h2 className="text-xl font-bold text-emerald-700">Pagamento riuscito!</h2>
            <p className="text-sm text-slate-600 mt-1">Grazie. I crediti sono stati accreditati sul tuo account.</p>
          </div>
          <div className="grid grid-cols-2 gap-4 w-full max-w-md">
            <div className="bg-white rounded-xl p-4 border border-slate-200">
              <p className="text-xs text-slate-500 uppercase font-medium">Crediti</p>
            ...
            <div className="bg-white rounded-xl p-4 border border-slate-200">
              <p className="text-xs text-slate-500 uppercase font-medium">Importo</p>
      ...
      {!error && !polling && order?.status === 'AWAITING_PAYMENT' && (
        <div className="glass rounded-2xl p-8 flex flex-col items-center text-center space-y-3 border-2 border-amber-200 bg-amber-50/50">
          ...
          <h2 className="font-semibold text-slate-900">Pagamento in elaborazione</h2>
          <p className="text-sm text-slate-600">
            La conferma da PagoPA non è ancora arrivata. Capita quando il pagamento viene processato in differita.
            Riceverai un aggiornamento appena disponibile; nel frattempo puoi consultare la cronologia.
          </p>
          <div className="flex gap-3 flex-wrap justify-center">
            <button onClick={() => navigate('/payments/history')} className="btn btn-primary inline-flex items-center gap-2">
              <ListChecks className="w-4 h-4" /> Vai alla cronologia
            </button>
      ...
      {!error && !polling && (order?.status === 'FAILED' || order?.status === 'CANCELED' || order?.status === 'EXPIRED') && (
        <div className="glass rounded-2xl p-8 flex flex-col items-center text-center space-y-3 border-2 border-red-200 bg-red-50/50">
          ...
          <h2 className="font-semibold text-slate-900">
            {order.status === 'FAILED' && 'Pagamento non riuscito'}
            {order.status === 'CANCELED' && 'Pagamento annullato'}
            {order.status === 'EXPIRED' && 'Posizione scaduta'}
          </h2>
          <p className="text-sm text-slate-600">
            Nessun credito è stato addebitato. Puoi riprovare in qualsiasi momento.
          </p>
          <div className="flex gap-3 flex-wrap justify-center">
            <button onClick={() => navigate('/credits/buy')} className="btn btn-primary inline-flex items-center gap-2">
              Riprova <ArrowRight className="w-4 h-4" />
            </button>
            <button onClick={() => navigate('/')} className="btn btn-secondary inline-flex items-center gap-2">
